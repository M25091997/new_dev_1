...
const PacketProducts = () => {
  ...
  const fetchProducts = async (isRefresh = false) => {
    try {
      ...
      const response = await getSellerPacketProducts(token, '', 'packet_products');

      if (response.status === 1) {
        setProducts(response.data?.products || []);
        setTotalItems(response.total || 0);
      } else {
        throw new Error(response.message || 'Failed to fetch packet products');
      }
    } catch (error) {
      console.error('Packet Products fetch error:', error);
      showError('Packet Products Error', error.message || 'Failed to load packet products');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.status === 1) {
        setProducts(response.data?.products || []);
        setTotalItems(response.total || 0);
        showSuccess('Refreshed', 'Packet products list has been refreshed and all filters cleared');
      } else {
        throw new Error(response.message || 'Failed to refresh packet products');
      }
    } catch (error) {
      console.error('Packet Products refresh error:', error);
      showError('Error', error.message || 'Failed to refresh packet products');
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handleAvailabilityToggle = async (product) => {
    try {
      ...
      // Call API to change availability status
      const response = await changeProductAvailabilityStatus(token, product.id);
    ...
    for (let i = startPage; i <= endPage; i++) {
      pages.push(
        <button
          ...
          className={`px-3 py-2 text-sm border border-gray-300 hover:bg-gray-50 ${i === currentPage
  ...
  const getAvailabilityToggle = (product) => {
    return (
      <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 ${product.status === 1 ? 'bg-green-500' : 'bg-gray-300'
          }`}
        title={product.status === 1 ? 'Available - Click to disable' : 'Unavailable - Click to enable'}
      >
        <span
          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${product.status === 1 ? 'translate-x-6' : 'translate-x-1'
            }`}
        />
      </button>
    );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <h1 className="text-xl mt-5 font-bold text-gray-800">Packet Products</h1>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            disabled={refreshing}
            className="flex items-center mt-5 space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
            <span>Refresh</span>
          </button>
          <button className="flex mt-5 items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">

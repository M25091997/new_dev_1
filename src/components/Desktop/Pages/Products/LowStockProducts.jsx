...
const LowStockProducts = () => {
  ...
  const fetchProducts = async (isRefresh = false) => {
    try {
      ...
      const response = await getSellerPacketProducts(token, '', 'low_stock');
      ...
      showError('Low Stock Products Error', error.message || 'Failed to load low stock products');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.status === 1) {
        setProducts(response.data?.products || []);
        setTotalItems(response.total || 0);
        showSuccess('Refreshed', 'Low stock products list has been refreshed and all filters cleared');
      } else {
        throw new Error(response.message || 'Failed to refresh low stock products');
      }
    } catch (error) {
      console.error('Low Stock Products refresh error:', error);
      showError('Error', error.message || 'Failed to refresh low stock products');
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const getAvailabilityToggle = (product, status) => {
    return (
      <button
        onClick={(e) => {
          e.stopPropagation();
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 ${status === 1 ? 'bg-red-500' : 'bg-gray-200'
          }`}
      >
        <span
                ...
                <tr key={product.product_variant_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-2">
                      ...
                      <button onClick={() => handleDeleteClick(product)} className="text-red-600 hover:text-red-900 p-1">

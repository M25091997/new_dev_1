...
const SoldOutProducts = () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.status === 1) {
        setProducts(response.data?.products || []);
        setTotalItems(response.total || 0);
        showSuccess('Refreshed', 'Sold out products list has been refreshed and all filters cleared');
      } else {
        throw new Error(response.message || 'Failed to refresh sold out products');
      }
    } catch (error) {
      console.error('Sold Out Products refresh error:', error);
      showError('Error', error.message || 'Failed to refresh sold out products');
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const getAvailabilityToggle = (status, product) => {
    return (
      <button
        onClick={(e) => {
          e.stopPropagation();
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <h1 className="text-2xl font-bold text-gray-800">Sold Out Products</h1>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedProducts.map((product) => (
                <tr key={product.product_variant_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {getAvailabilityToggle(product.status, product)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
